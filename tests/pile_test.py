...
class PileTest(unittest.TestCase):
    ...
    def test_add_cards_to_pile(self):
        ...
        card_name = card_response["cards"][0]['code']
        response = self.pile.add_to_pile(deck_id=self.deck_id, name=self.pile_name, cards=card_name)
        ...
        self.assertEqual(cards_in_pile["piles"][self.pile_name]["cards"][0]["code"],card_name)
    ...
    def test_draw_cards_to_pile(self):
        ...
        card_response = self.deck.draw_card(deck_id=self.deck_id, count=1).json()
        card_name = card_response["cards"][0]['code']
        self.pile.add_to_pile(deck_id=self.deck_id, name=self.pile_name, cards=card_name)
        discared_card_response=self.pile.draw_from_pile(deck_id=self.deck_id,name=self.pile_name,card=card_name)
        self.assertTrue(discared_card_response.status_code, 'Didnt get status 200 OK')
        resp_json = discared_card_response.json()
        self.assertEqual(card_name,resp_json["cards"][0]["code"])
        cards_in_pile = self.pile.list_cards_in_pile(deck_id=self.deck_id, name=self.pile_name).json()
